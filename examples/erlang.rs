...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // ---- 初期化する ----
    // 乱数生成器
    let mut generator = rand_simple::Erlang::new([1192u32, 765u32, 1543u32]);
    // CSVファイルのパス
    let file_path = "C:/MyPrograms/demo_rand_simple/data/Erlang.csv";
    ...
    let _: Result<(i64, f64), &str> = generator.try_set_params(shape, scale);
