...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // ---- 初期化する ----
    // 乱数生成器
    let mut generator = rand_simple::Frechet::new(1192_u32);

    // CSVファイルのパス
    let file_path = "C:/MyPrograms/demo_rand_simple/data/Frechet.csv";
    ...
    let _: Result<(f64, f64), &str> = generator.try_set_params(shape, scale);
